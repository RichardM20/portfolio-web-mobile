import 'package:flutter/material.dart';
import 'package:personal_portfolio/utils/colors_app.dart';

const TextStyle bigPrimaryTitleStyle = TextStyle(
  fontSize: 30,
  fontFamily: "RobotoCondensed-Light",
  color: color,
);
const TextStyle bigSecondayTitleStyle = TextStyle(
  fontSize: 30,
  fontFamily: "RobotoCondensed-Regular",
  color: primaryColor,
);
const TextStyle mediumPrimaryTextStyle = TextStyle(
  fontSize: 20,
  fontFamily: "RobotoCondensed-Light",
  color: color,
);
const TextStyle mediumSecondaryTextStyle = TextStyle(
  fontSize: 20,
  fontFamily: "RobotoCondensed-Regular",
  color: primaryColor,
);
const TextStyle paragraphTextStyle = TextStyle(
  fontSize: 10,
  fontFamily: "RobotoCondensed-Light",
  color: color,
);
const TextStyle paragraphSecondayTextStyle = TextStyle(
  fontSize: 10,
  fontFamily: "RobotoCondensed-Regular",
  color: color,
);
const TextStyle buttonsTextStyle = TextStyle(
  fontSize: 10,
  fontFamily: "RobotoCondensed-Regular",
  color: Colors.white,
);
const TextStyle buttonsSecondaryTextstyle = TextStyle(
  fontSize: 10,
  fontFamily: "RobotoCondensed-Regular",
  color: primaryColor,
);
const TextStyle subtitlePrimaryStyle = TextStyle(
  fontSize: 12,
  fontFamily: "RobotoCondensed-Regular",
  color: primaryColor,
);
const TextStyle subtitleSecondaryStyle = TextStyle(
  fontSize: 12,
  fontFamily: "RobotoCondensed-Light",
  color: color,
);
const TextStyle smallParagraphStyle = TextStyle(
  fontSize: 8,
  fontFamily: "RobotoCondensed-Regular",
  color: Colors.white,
);
const TextStyle smallParagraphSecondaryStyle = TextStyle(
  fontSize: 6,
  fontFamily: "RobotoCondensed-Light",
  color: Colors.white,
);
